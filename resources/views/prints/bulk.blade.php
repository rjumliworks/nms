        ...
        @foreach($lists as $date => $list)
            ...
            <table style="border: 1px solid black; font-size: 10px; margin-top: 22px;">
                <thead style="background-color:#c8c8c8; padding: 5px; font-size: 9px;">
                    <tr>    
                        <th style="vertical-align: middle;" width="40%">Employee</th>
                        <th style="vertical-align: middle;" width="15%">AM Time-In</th>
                        <th style="vertical-align: middle;" width="15%">AM Time-Out</th>
                        <th style="vertical-align: middle;" width="15%">PM Time-In</th>
                        <th style="vertical-align: middle;" width="15%">PM Time-Out</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach($list as $dtr)  <!-- 🔥 THIS IS THE FIX -->
                      <tr>
                        <td>{{ $dtr['name'] ?? '' }}</td>

                        <td style="text-align: center;">
                            {{ isset($dtr['am_in_at']->time) 
                                ? \Carbon\Carbon::parse($dtr['am_in_at']->time)->format('h:i A') 
                                : '-' }}
                        </td>

                        <td style="text-align: center;">
                            {{ isset($dtr['am_out_at']->time) 
                                ? \Carbon\Carbon::parse($dtr['am_out_at']->time)->format('h:i A') 
                                : '-' }}
                        </td>

                        <td style="text-align: center;">
                            {{ isset($dtr['pm_in_at']->time) 
                                ? \Carbon\Carbon::parse($dtr['pm_in_at']->time)->format('h:i A') 
                                : '-' }}
                        </td>

                        <td style="text-align: center;">
                            {{ isset($dtr['pm_out_at']->time) 
                                ? \Carbon\Carbon::parse($dtr['pm_out_at']->time)->format('h:i A') 
                                : '-' }}
                        </td>
                    </tr>
                    @endforeach
